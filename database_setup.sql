...
-- ============================================================
-- ADMINISTRACIÓN SEGURA DE CRÉDITOS DE IA
-- Mantener sincronizado con 202609250001_admin_credit_management.sql.
-- ============================================================
...
BEGIN
    ...
    SELECT p.id, p.email, p.username, p.role,
           COALESCE(w.plan_id, 'free'), COALESCE(w.balance, 0), w.updated_at
...
BEGIN
    ...
    SELECT balance INTO v_old_balance FROM public.ai_credit_wallets
    ...
    ELSE
        UPDATE public.ai_credit_wallets
        SET balance = p_balance, updated_at = timezone('utc', now())
        WHERE user_id = p_user_id;
    END IF;

    INSERT INTO public.security_logs (user_id, action, details)
    VALUES (v_admin_id, 'AI_CREDITS_ADMIN_UPDATE',
        format('Cuenta %s (%s): %s -> %s créditos. %s', p_user_id, v_target_email, v_old_balance, p_balance, v_reason));

    RETURN jsonb_build_object('ok', TRUE, 'code', 'CREDITS_UPDATED',
        'userId', p_user_id, 'previousBalance', v_old_balance, 'balance', p_balance);
END;
$$;
